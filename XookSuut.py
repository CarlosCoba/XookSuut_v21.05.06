...
errors	=	Boolean. If you want to compute errors via MCMC for the derived parameters.
...
if __name__ == "__main__":
	...
	if (nargs == 30 ):
		...
		rotcur(galaxy, vel_map, evel_map, SN, VSYS, PA, INC, X0, Y0, n_it, pixel_scale, vary_PA, vary_INC, vary_XC, vary_YC, vary_VSYS, vary_PHI, delta, rstart, rfinal, ring_space, frac_pixel, 		bar_min_max,vmode, save_plots, errors, survey,config,e_ISM)


	else:
		print ("USE: XookSuut.py object vel_map [evel_map] SN [VSYS] PA INC X0 Y0 [N_it=5] [pixel_scale=1] vary_PA[0,1] vary_INC[0,1] vary_XC[0,1] vary_YC[0,1] vary_VSYS[0,1] vary_THETA_bar[0,1] ring_space [Delta] [Rstart=2.5] [Rfinal=40] [frac_pixel=2/3.] [R_bar_min,R_bar_max] [model=cicular,radial,bisymmetric] [save_plots=1] [errors=0] [survey] [config_file] [e_ISM=5]")
...
XookSuut.py test_galaxy test.fits ""  20 "" 200 50 27 27 5 1 1 1 1 1 1 1 2.5 "" 3 46 2/3. 3,20 bisymmetric 1 0 test "" 5
